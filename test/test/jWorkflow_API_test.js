module( "jWorkflow API test" );

test( "we can place an order", function(){
  expect( 1 );
...
test( "order throws an error when not given a function", function(){
  expect( 1 );
  var errored = false;
  try {
    jWorkflow.order( 42 );
...
test( "we can call andThen on the return value of order", function(){
  expect( 2 );
  ...
  equal( typeof(order.andThen), "function", "expected andThen to be a function" );
} );

test( "andThen throws an error when not given a function", function(){
  expect( 1 );
...
test( "we can call andThen on the return value of andThen", function(){
  expect( 2 );
...
test( "it doesnt invoke the order function when start isnt called", function(){
  var dude = true,
    sweet = function(){
    dude = false;
  },
...
test( "it calls the order function when start is called", function(){
  var dude = false,
    sweet = function(){
    dude = true;
  },
...
asyncTest( "it can handle multiple orders without mixing them", function(){
  expect( 1 );
  var dude = false,
    what = false,
    sweet = function(){
...
test( "it calls the order in the order that it was built", function(){
  expect( 1 );
  ...
  deepEqual( [ "garlicChicken", "whiteRice", "wontonSoup", "cookiesFortune", "noAndThen", "noAndThen" ], result, "expected functions to be called in order" );
} );

test( "it get the return value of the previous func", function(){

  var dude = function(){
    return 42;
  },
    sweet = function( previous ){
    equal( previous, 42, "expected previous to be return value" );
  },
...
test( "we get a baton play with", function(){
...
test( "when I take the baton, the next methods are not called if I don't pass it", function(){
...
asyncTest( "when I take the baton and pass it async, the next methods are called", function(){
  expect( 1 );
...
asyncTest( "it calls the order in the order that it was built, even with Async calls", function(){
  expect( 1 );
  ...
  order.start( function(){
    start();
    deepEqual( result, [ "garlicChicken", "whiteRice", "wontonSoup", "cookiesFortune", "noAndThen", "noAndThen" ], "expected functions to be called in order" );
  } );
} );

test( "we can give context for the function passed to andThen", function(){
  expect( 1 );
...
test( "we can pass in a different context for each function", function(){
  expect( 2 );
...
test( "we can pass context into start", function(){
  expect( 1 );
...
test( "we can pass null into order", function(){
  expect( 1 );
...
test( "we can NOT pass null into andThen", function(){
  expect( 1 );
  var errored = false,
    order = jWorkflow.order();
...
asyncTest( "we can pass a return value from an async function into a sync function via the baton", function(){
  expect( 1 );
...
asyncTest( "we can reuse an order", function(){
  expect( 1 );
  ...
  order.start( function(){
    order.start( function(){
      start();
      equal( sheep_teleported, 4, "expected to teleport 4 sheep" );
    } );
  } );
} );

asyncTest( "we can chill for a little bit between tasks", function(){
  var time = [ ],
    inhale = function(){
    time.push( new Date() );
  },
...
test( "it passes the final result to the callback passed to start", function(){
  expect( 1 );
...
test( "we can pass the initial value into start", function(){
  expect( 1 );
...
asyncTest( "we can pass another workflow into andThen", function(){
  expect( 1 );
...
asyncTest( "we can pass an array of functions into andThen", function(){
  expect( 1 );
...
asyncTest( "we can pass an array of workflows into andThen", function(){
  expect( 1 );
...
asyncTest( "we can stop execution of a workflow with baton.drop", function(){
  expect( 2 );
